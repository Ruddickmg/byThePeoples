...
//         let request_data = fake::NameRequest::new(&name, &password);
//         helper
...
//             .to_http_parts();
//         let json = web::Json::<fake::NameRequest>::from_request(&req, &mut payload)
//             .await
//             .unwrap();
...
//         let (name, email, password) = test_helper::fake_credentials();
//         let hashed_password = controller::password::hash_password(&password).unwrap();
//         let data = fake::FullRequest::new(&name, &hashed_password, &email);
//         helper.add_credentials(&data).await;
//         let request_data = fake::NameRequest::new(&name, "invalid password");
//         let (req, mut payload) = test::TestRequest::post()
//             .set_json(&request_data)
//             .to_http_parts();
//         let json = web::Json::<fake::NameRequest>::from_request(&req, &mut payload)
