...
impl<'a> Client<'a> {
    pub fn new(client: PooledConnection<'a>) -> GenericClient {
    ...
    pub async fn query<'b>(self, query: &Statement, params: Params<'b>) -> Result<Results> {
        Ok(self.client.query(query, params).await?)
    }
    pub async fn transaction(&'a mut self) -> Result<GenericTransaction<'_>> {
